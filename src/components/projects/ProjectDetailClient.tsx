...
const ProjectDetailClient: FC<Props> = ({ project }) => {
  const { locale, t } = useI18n();
  const statusLabel =
    project.status === "developing" ? t("statusDeveloping") : t("statusActive");
  return (
    <Container className="py-16 space-y-10 max-w-prose">
      <header className="space-y-3">
        {project.highlight && (
          <span
            className={`inline-flex w-max text-[11px] px-3 py-1 rounded-full font-semibold uppercase tracking-wide ${project.highlight.colorClass}`}
          >
            {project.highlight.label[locale]}
          </span>
        )}
      ...
      <Prose>
        ...
        <h2 className="text-xl mt-10 mb-2 font-semibold tracking-tight">
          {locale === "en" ? "Goal" : "Objetivo"}
        </h2>
