...
const ExperienceTimeline: React.FC = () => {
  ...
  const formatRange = (start: string, end?: string | null) => {
    ...
    const e = end ? new Date(end).toLocaleDateString(locale, opts) : t("present");
    return `${s} – ${e}`;
  };
  ...
  return (
    <Section id="experience" title={t("experienceTitle")}>     
      <Timeline position="right" sx={{
        ...
        '& .MuiTimelineConnector-root': { backgroundColor: 'hsl(var(--border))' },
      }}>
        {careers.map((c, idx) => {
          ...
          return (
            <TimelineItem key={key}>
              <TimelineSeparator>
                <TimelineDot variant={idx === 0 ? 'filled' : 'outlined'} color={idx === 0 ? 'primary' : 'grey'} sx={{
                  backgroundColor: idx === 0 ? 'hsl(var(--primary))' : 'hsl(var(--accent-2)/0.5)',
                ...
                <div className="pb-8 last:pb-0">
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                    <h3 className="font-medium tracking-tight text-sm sm:text-base">
                      {c.position[locale]} <span className="text-[hsl(var(--color-muted))]">@ {c.company}</span>
                    </h3>
                    <span className="ms-auto text-[11px] font-mono text-[hsl(var(--color-muted))]">{formatRange(c.start, c.end)}</span>
                  </div>
