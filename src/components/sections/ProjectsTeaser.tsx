        ...
        {list.map((p) => (
          <li key={p.slug} className="rounded-lg border border-[hsl(var(--border))] p-4 flex flex-col gap-1 bg-[hsl(var(--card))]">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
              <span className="font-medium">{p.title[locale]}</span>
              ...
              {p.status === "developing" && (
                <span className="text-[10px] px-2 py-0.5 rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--accent-2)/0.3)] tracking-wide uppercase">{locale === 'en' ? 'Developing' : 'En desarrollo'}</span>
              )}
